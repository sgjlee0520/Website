...
\begin{align}
    \mathbf{x}_d = \left(M_d,\; \sigma_{P,d},\; \sigma_{t,d},\; U_d,\; U_{\mathrm{norm},d},\; \langle t \rangle_d \right)
\end{align}
...
\midrule
$U_\mathrm{norm}$              & $-6.025$ & $0.000$ & stationary     & $0.512$, $p=0.039$ & non-stationary \\
$\log(U_\mathrm{norm})$        & $-6.266$ & $0.000$ & stationary     & $0.457$, $p>0.05$  & stationary \\
$\Delta\log(U_\mathrm{norm})$  & $-15.89$ & $0.000$ & stationary     & $0.027$, $p>0.10$  & stationary \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Sample size caveat:} $n = 10$ predictions is far too small for statistical inference. A binomial test of $H_0: p = 0.5$ against $H_1: p > 0.5$ gives $p$-value $= \binom{10}{7+8+9+10} \cdot 0.5^{10} \approx 0.172$ — not significant at any conventional level. A minimum of $\sim$30--50 graded predictions is needed before drawing conclusions.
\end{itemize}
