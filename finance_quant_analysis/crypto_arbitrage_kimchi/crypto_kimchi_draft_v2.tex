...
\begin{equation}
V_{0} = \tau_{1} + \tau_{2} + \tau_{3} = 0.05\% + 0.25\% + 0.05\% = 0.35\%.
\end{equation}
...
Let $\Pi^{\star}_{t}$ denote the global best-minute operator, which is already an optimistic upper envelope over assets and directions. For each threshold $\tau$, define
\begin{equation}
\phi(\tau) = P\!\left(\Pi^{\star}_{t} > \tau\right).
\end{equation}
...
\begin{itemize}
    \item $\phi(0.15\%) = P\!\left(\Pi^{\star}_{t} > 0.15\%\right) = 0.000141$
    \item $\phi(0.35\%) = P\!\left(\Pi^{\star}_{t} > 0.35\%\right) = 0$
    \item $\phi(0.40\%) = P\!\left(\Pi^{\star}_{t} > 0.40\%\right) = 0$
    \item $\phi(0.417\%) = P\!\left(\Pi^{\star}_{t} > 0.417\%\right) = 0$
    \item $\phi(0.75\%) = P\!\left(\Pi^{\star}_{t} > 0.75\%\right) = 0$
\end{itemize}
...
#    (both lines use the same flags: -interaction=nonstopmode -halt-on-error)
